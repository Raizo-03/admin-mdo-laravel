...
<div class="mb-4 text-left">
    <button 
        ...
        @if($cantAddDoctor) opacity-50 cursor-not-allowed @endif"
        @if($cantAddDoctor) disabled @endif
    >
        + Add Doctor
    </button>
</div>
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        <div class="overflow-x-auto p-4">
            ...
            <div class="mb-4 text-sm text-gray-600">
                Showing {{ $admins->count() }} of {{ $admins->total() }} doctors
                @if($statusFilter !== 'all')
           ...
           @else
            ...
            <div class="text-center py-8">
                <div class="text-gray-500 text-lg mb-2">📋 No doctors found</div>
        ...
        <div id="editModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
            <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                <h2 class="text-gray-900 text-xl font-semibold mb-4">Edit Doctor</h2>
...
<script>
    ...
    const downloadUrl = `{{ route('doctors.export') }}?${params.toString()}`;
    ...
    .then(blob => {
        ...
        const filename = format === 'excel' ? `Doctors_${dateStr}.csv` : `Doctors_${dateStr}.csv`;
        a.download = filename;
